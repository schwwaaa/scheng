...
- [x] scheng-output-syphon: SyphonSink, Metal bridge (ObjC), build.rs, FFI
- [~] scheng-output-spout: SpoutSink + ffi.rs ready — needs native bridge ported from runtime-glow
- [x] scheng-output-ffmpeg: FfmpegSink, FfmpegWorker, FfmpegConfig, bounded queue, RTSP/RTMP/File
- [~] scheng-output-ndi: NdiSink + NdiConfig ready — needs NDI SDK Rust bindings wired in
- [x] Headless/offscreen OutputSink: PixelReadbackSink in scheng-runtime-wgpu (Phase 1)
