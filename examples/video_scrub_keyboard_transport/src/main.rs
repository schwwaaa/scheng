// examples/video_scrub_keyboard_transport/src/main.rs
//
...
fn make_gl(
    ...
    let display_builder = DisplayBuilder::new().with_window_builder(Some(
        winit::window::WindowBuilder::new()
            .with_title("scheng: video_scrub_keyboard_transport")
...
fn main() {
    ...
    props.shader_sources.insert(
        ...
        rt::ShaderSource {
            ...
            origin: Some("video_scrub_keyboard_transport:passthrough".into()),
        },
    );
    ...
    let cfg_path = std::env::args().nth(1).unwrap_or_else(|| {
        eprintln!(
            "Usage: scheng-example-video-scrub-keyboard-transport <video_config.json>"
        );
        std::process::exit(2);
    });

    // --- DEBUG: verify JSON + video file before handing off to engine ----
    println!("[config] using video config JSON: {}", cfg_path);
    match std::fs::read_to_string(&cfg_path) {
        Ok(text) => {
            println!("[config] json length: {} bytes", text.len());
            match serde_json::from_str::<serde_json::Value>(&text) {
                Ok(v) => {
                    if let Some(file) = v.get("file").and_then(|f| f.as_str()) {
                        println!("[config] video file in JSON: {}", file);
                        let p = std::path::Path::new(file);
                        if p.exists() {
                            println!("[config] video file exists on disk");
                        } else {
                            println!("[config] WARNING: video file NOT found on disk");
                        }
                    } else {
                        println!("[config] WARNING: JSON has no 'file' string field");
                    }
                }
                Err(e) => {
                    println!("[config] WARNING: failed to parse JSON as object: {e}");
                }
            }
        }
        Err(e) => {
            println!("[config] WARNING: failed to read JSON file: {e}");
        }
    }
    // ---------------------------------------------------------------------
        ...
        match event {
            ...
            Event::RedrawRequested(_) => {
                ...
                unsafe {
                    ...
                    // If this ever logs a weird size, we know something
                    // upstream is off — but 960x540 here is sane.
                    println!("[debug] main_out {}x{}", main_out.width, main_out.height);
