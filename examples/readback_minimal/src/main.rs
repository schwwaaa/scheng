...
/// Presents an ExecOutput to the window backbuffer (for live preview).
/// Uses FBO -> default framebuffer blit.
...
fn build_graph(props: &mut rt::NodeProps) -> graph::Graph {
    ...
    props.shader_sources.insert(
        ...
        rt::ShaderSource {
            ...
            origin: Some("readback_minimal".to_string()),
        },
    );
...
fn main() {
    ...
    let display_builder = DisplayBuilder::new().with_window_builder(Some(
        WindowBuilder::new()
            .with_title("scheng readback minimal")
    ...
    // Live preview (window backbuffer).
    ...
    // CPU readback sink.
    // NOTE: stride is bytes-per-row; for RGBA8 it's width*4.
    let mut readback = rt::ReadbackSink::new((RENDER_W as u64) * 4);
        ...
        *control_flow = ControlFlow::Poll;
        ...
        match event {
            ...
            Event::MainEventsCleared => {
                ...
                // 1) read back to CPU (for recording/export pipelines next)
                readback.consume(&gl, main_out);

                // 2) show on-screen so it's easy to debug
