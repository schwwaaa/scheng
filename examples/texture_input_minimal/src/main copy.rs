...
fn main() {
    ...
    // Connect tex_in.main -> out.main
    let from = graph::Endpoint { node: tex_in, port: graph::PortId(0), dir: graph::PortDir::Out };
    let to   = graph::Endpoint { node: out,    port: graph::PortId(0), dir: graph::PortDir::In  };
    g.connect(from, to).expect("connect");

    let plan = g.compile().expect("compile plan");

    let mut props = rt::NodeProps::default();
    props.output_names.insert(out, "main".into());
