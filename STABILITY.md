...
This repository is an SDK. Stability is defined by **contract crates** and by explicit written policy.

## Contract crates (public SDK surface)
These crates define the developer contract and must remain dependency-pure and stable:
...
Guarantees:
- No OS/windowing dependencies
- No GPU/backend handles
- Stable data model + validation semantics
- Contract tests + CI gates must pass

## Backend crates (consumers of the contract)
These crates implement execution and may evolve independently, but **must obey the contract crates**:

- `scheng-runtime-glow`

Rules:
- Backends must not invent new graph semantics.
- If a capability is not in `scheng-graph` + `scheng-runtime`, a backend cannot assume it exists.
- Backend-only additions must not leak into contract crates.

## Host / integration crates (non-contract)
These crates may depend on platform/runtime libraries and are not the SDK contract:
...
## CI is law
A change that breaks any of the following is considered a contract break:
- forbidden dependency gates
- `sdk-compat`
