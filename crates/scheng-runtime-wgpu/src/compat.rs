...
pub fn process(user_frag: &str, node_label: &str) -> ProcessedShader {
    ...
    if !custom_uniform_names.is_empty() {
        // Phase 1.2: pack custom u_* uniforms into CustomBlock at binding 6.
        // Strip the individual uniform declarations — they're replaced by the block.
        src = RE_CUSTOM_UNIFORM.replace_all(&src, "").into_owned();
        // Inject CustomBlock declaration with all discovered uniforms
        let mut custom_block = String::from(
        ...
        // Insert after the compat header (before user shader body)
