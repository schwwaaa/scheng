...
use scheng_runtime_wgpu::executor::NodeConfig;
...
impl NodeConfigBuilder {
    ...
    pub fn build(&self, store: &ParamStore) -> HashMap<NodeId, NodeConfig> {
        ...
        // Ensure every registered node has a base config.
        ...
        // Phase 1.2 hook: when NodeConfig gains a `uniforms` field,
        // populate it here from store.all_values() filtered by node_label.
        //
        ...
        //                 configs.get_mut(&nid).map(|c| c.uniforms.insert(param.name, value));
        //             }
        //         }
        //         None => {
        ...
        let _ = store; // suppress unused warning until Phase 1.2
