...
use crate::assets::{
    load_json_result, pick_platform_json, read_to_string_result, resolve_assets_path, AssetsRoot,
};
...
/// Resolved, OS-aware config paths for a scheng run.
#[derive(Debug, Clone)]
...
fn load_render_selection_with_mode(
    ...
    let (version, frag, frag_variants_s, active_frag, present_frag, frag_profile_map_s) = match mode
    {
        ...
        ConfigMode::Strict => {
            let rj: RenderJsonStrict =
                serde_json::from_str(&data).map_err(|e| EngineError::Json {
                    path: render_cfg.clone(),
                    source: e,
                })?;
            (
...
pub fn parse_loaded_json<T: serde::de::DeserializeOwned>(
    loaded: &LoadedJson,
...
fn load_engine_config_from_mode(
    start_dir: &Path,
    mode: ConfigMode,
...
pub fn load_typed_json<T: serde::de::DeserializeOwned>(
    path: &std::path::Path,
) -> Result<T, EngineError> {
